 ...
 * Used for relatively static lookups (product categories, FAQ, brand config)
 * to reduce load on downstream services.
 *
...
export class CacheService {
   ...
   * If ttl is omitted, defaults to 5 minutes.
   */
  set(key: string, value: any, ttlMs?: number): void {
    const ttl = ttlMs ?? parseInt(process.env.CUSTOMER_PORTAL_CACHE_TTL_MS || '300000', 10);
