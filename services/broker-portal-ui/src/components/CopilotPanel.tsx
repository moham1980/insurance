...
export function CopilotPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  const [messages, setMessages] = React.useState<Message[]>([
    { role: 'assistant', content: 'سلام! من دستیار هوشمند پورتال کارگزاری هستم. چطور می‌توانم کمک کنم؟' },
  ]);
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      const res = await postBFF('/broker/copilot/chat', { message: text, conversationHistory });
      ...
      <div className="flex items-center justify-between border-b border-border-default bg-gradient-to-l from-brand-primary to-brand-secondary px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-text-on-brand/80">پورتال کارگزاری</p>
          </div>
        </div>
