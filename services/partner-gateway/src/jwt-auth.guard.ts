...
/**
 * JWT auth guard for partner-gateway management endpoints.
 * Validates Bearer token signature using JWKS (RS256) or local secret (HS256).
 */
@Injectable()
export class JwtAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      this.logger.debug(`JWKS validation failed, falling back to local JWT: ${(jwksErr as any)?.message || jwksErr}`);
    }
