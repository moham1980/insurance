...
async function bootstrap() {
  ...
  if (typeof kafkaBrokers === 'string' && kafkaBrokers.trim().length > 0) {
    const logger = createLogger({ serviceName: 'workflow-engine-service', level: process.env.LOG_LEVEL || 'info' });
    const producer = new KafkaProducer(
      {
        brokers: kafkaBrokers.split(',').map((x) => x.trim()).filter(Boolean),
        clientId: 'workflow-engine-service',
      },
      logger,
    );
    ...
    const worker = new OutboxWorker({
      ...
      producer,
      logger,
      producerName: 'workflow-engine-service',
