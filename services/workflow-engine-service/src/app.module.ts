...
@Module({
  imports: [
    TypeOrmModule.forRoot({
      ...
      entities: [ProcessDefinition, ProcessInstance, ProcessToken, ProcessVariable, ProcessHistory, ProcessTimer, OutboxEvent, ConsumedEvent, DeadLetterEvent],
    }),
    TypeOrmModule.forFeature([ProcessDefinition, ProcessInstance, ProcessToken, ProcessVariable, ProcessHistory, ProcessTimer, OutboxEvent, ConsumedEvent, DeadLetterEvent]),
    HttpModule,
  ],
  controllers: [WorkflowEngineController, HealthController],
  providers: [AbacGuard, TenantGuard, WorkflowEngineService, JwtAuthGuard, PermissionsGuard, {
