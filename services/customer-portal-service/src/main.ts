...
async function bootstrap() {
  ...
  if (kafkaBrokers.length > 0) {
    ...
    const logger = createLogger({ serviceName: 'customer-portal', level: process.env.LOG_LEVEL || 'info' });
    const kafkaProducer = new KafkaProducer({ brokers: kafkaBrokers, clientId: 'customer-portal' }, logger);
    await kafkaProducer.connect();
    const outboxWorker = new OutboxWorker({
      dataSource,
      producer: kafkaProducer,
      logger,
      producerName: 'customer-portal',
    ...
    outboxWorker.start();
    console.log('OutboxWorker started for customer-portal');
  }

app.useGlobalFilters(new AllExceptionsFilter());
await app.listen({ port, host: '0.0.0.0' });
  console.log(`Customer Portal Service listening on port ${port}`);
}
