...
async function bootstrap() {
  ...
  if (kafkaBrokers.length > 0) {
    ...
    const logger = createLogger({ serviceName: 'knowledge', level: process.env.LOG_LEVEL || 'info' });
    const kafkaProducer = new KafkaProducer({ brokers: kafkaBrokers, clientId: 'knowledge' }, logger);
    await kafkaProducer.connect();
    const outboxWorker = new OutboxWorker({
      dataSource,
      producer: kafkaProducer,
      logger,
      producerName: 'knowledge',
    ...
    outboxWorker.start();
    console.log('OutboxWorker started for knowledge');
  }
