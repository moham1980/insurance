...
async function bootstrap() {
  ...
  if (typeof kafkaBrokers === 'string' && kafkaBrokers.trim().length > 0) {
    const logger = createLogger({ serviceName: 'party-kyc-service', level: process.env.LOG_LEVEL || 'info' });
    ...
    const worker = new OutboxWorker({
      ...
      producer,
      logger,
      producerName: 'party-kyc-service',
