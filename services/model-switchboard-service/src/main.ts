...
async function bootstrap() {
  ...
  if (kafkaBrokers.length > 0) {
    ...
    const logger = createLogger({ serviceName: 'model-switchboard', level: process.env.LOG_LEVEL || 'info' });
    const kafkaProducer = new KafkaProducer({ brokers: kafkaBrokers, clientId: 'model-switchboard' }, logger);
    await kafkaProducer.connect();
    const outboxWorker = new OutboxWorker({
      dataSource,
      producer: kafkaProducer,
      logger,
      producerName: 'model-switchboard',
