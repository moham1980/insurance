...
async function bootstrap() {
  ...
  if (kafkaBrokers.length > 0) {
    ...
    const logger = createLogger({ serviceName: 'agent-portal', level: process.env.LOG_LEVEL || 'info' });
    const kafkaProducer = new KafkaProducer({ brokers: kafkaBrokers, clientId: 'agent-portal' }, logger);
    await kafkaProducer.connect();
    const outboxWorker = new OutboxWorker({
      dataSource,
      producer: kafkaProducer,
      logger,
      producerName: 'agent-portal',
    ...
    outboxWorker.start();
    console.log('OutboxWorker started for agent-portal');
  }

await app.listen({ port, host: '0.0.0.0' });
  console.log(`Agent Portal Service listening on port ${port}`);
}
