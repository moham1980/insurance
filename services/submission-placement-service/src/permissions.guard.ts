...
export class PermissionsGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!user) {
      throw new UnauthorizedException({ success: false, error: { code: 'UNAUTHORIZED', message: 'Unauthorized' } });
    }

    const perms = permissionsForRoles(user.roles);
    const ok = required.every((p) => perms.includes(p));
    if (!ok) {
      throw new ForbiddenException({ success: false, error: { code: 'FORBIDDEN', message: 'Forbidden' } });
    }
    return true;
  }
}
