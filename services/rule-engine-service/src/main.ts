...
async function bootstrap() {
  ...
  if (kafkaBrokers.length > 0) {
    ...
    const logger = createLogger({ serviceName: 'rule-engine', level: process.env.LOG_LEVEL || 'info' });
    const kafkaProducer = new KafkaProducer({ brokers: kafkaBrokers, clientId: 'rule-engine' }, logger);
    await kafkaProducer.connect();
    const outboxWorker = new OutboxWorker({
      dataSource,
      producer: kafkaProducer,
      logger,
      producerName: 'rule-engine',
    ...
    outboxWorker.start();
    console.log('OutboxWorker started for rule-engine');
  }
