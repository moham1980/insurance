 ...
 * Format: 10 digits
 */
export function redactNationalId(nationalId, options = {}) {
 ...
 */
export function redactIban(iban, options = {}) {
    const opts = { ...defaultOptions, ...options };
    if (!iban || !iban.startsWith('IR') || iban.length !== 26)
        return iban;
    if (opts.preserveLength) {
        const prefix = 'IR';
 ...
 */
export function redactCardNumber(cardNumber, options = {}) {
    const opts = { ...defaultOptions, ...options };
    if (!cardNumber)
        return cardNumber;
 ...
 */
export function redactPhoneNumber(phoneNumber, options = {}) {
    const opts = { ...defaultOptions, ...options };
    if (!phoneNumber)
        return phoneNumber;
 ...
 */
export function redactEmail(email, options = {}) {
    const opts = { ...defaultOptions, ...options };
 ...
 */
export function redactPiiInObject(obj, options = {}) {
 ...
 */
export function minimizeData(obj, allowedFields) {
    if (!obj || typeof obj !== 'object')
        return obj;
    const result = {};
